...
export default function Clients() {
  ...
  const handleViewBillingAndPayments = (clientId: string) => {
  ...
  const handleDelete = async (id: string) => {
    const confirmed = await confirmDelete(
      'Remove Client?',
      'This will permanently delete this client from the directory.'
    );
    if (!confirmed) return;
    ...
    try {
      ...
      await Promise.race([
        showError(
          'Deletion Failed',
          err.message || 'Unable to delete the client. Please try again.'
        ),
        new Promise((resolve) => setTimeout(resolve, 1000))
      ]);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.message || `Failed to ${isEditing ? 'update' : 'create'} client`);
      }
  ...
  const columns = [
    {
      ...
      render: (c: Client) => (
        <div className="flex items-center gap-1.5">
          <button
            onClick={() => handleViewBillingAndPayments(c.id)}
    ...
    },
    {
      ...
      sortable: true,
      render: (c: Client) => {
        const dueNum = typeof c.total_due === 'string' ? parseFloat(c.total_due) : c.total_due;
        const isCredit = dueNum < 0;
        return (
          <div className={`font-semibold inline-flex items-center gap-1 px-2.5 py-1 rounded-lg ${isCredit
            ? 'text-emerald-600'
            : dueNum > 0
              ? 'text-rose-700'
              : 'text-slate-600'
            }`}>
            {formatCurrency(Math.abs(dueNum))}
            {isCredit && <span className="uppercase font-semibold tracking-tight">(Credit)</span>}
            {dueNum > 0 && <span className="uppercase font-semibold tracking-tight">(Due)</span>}
          </div>
        );
      }
    },
      ...
      <div className='bg-white shadow-sm shadow-amber-100 p-5 rounded-xl'>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-1">Manage client details, track billing, milestone payments, and live balances.</p>
          </div>
          <button
            onClick={() => {
              setEditId(null);
        ...
        <div className='mt-2'>
          ...
          {loading ? (
            ...
            <Table<Client>
              ...
              columns={columns}
              keyExtractor={(c) => c.id}
              emptyMessage="No clients found in database. Get started by adding one above."
              lazy
              ...
              >
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-xl font-bold text-slate-900">
                    {editId ? 'Update Client Details' : 'Add New Client'}
                  </h3>
                  <button
                    onClick={handleCloseModal}
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    <label className="block text-xs font-semibold uppercase text-slate-500 mb-1">Client Name <span className='text-red-600'>*</span></label>
                    <input
                      type="text"
                      required
                      value={formData.name}
